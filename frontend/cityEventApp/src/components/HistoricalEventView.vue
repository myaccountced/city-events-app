...
const historicPath = ref("/historic");
...
const loadMoreEventWithFilterAndSorter = async () => {
  ...
  // Load more events with optional filter and sorter that are historic
  await getEventsWithFilterAndSorter(limit, offset.value, curFilters.value, curSorter.value, true, prevSearch.value); // Fetch events with pagination
  autocompleteEvents.value = events.value;
...
const clearSearch = async () => {
  ...
  for (let index in curFilters.value) {
    let fieldName = curFilters.value[index].fieldName;

    if (fieldName != 'moderatorApproval') {
      fieldName = fieldName.toString().replace("event", "");
      fieldName = fieldName.toString().charAt(0).toLowerCase() + fieldName.toString().substring(1);
      queryObject[fieldName] = curFilters.value[index].criteria;
    }
  }

  await router.push({ path: `${historicPath.value}`, query: queryObject })
}
...
const handleSearchQueryChange = () => {
  ...
  router.push({ path: `${historicPath.value}`, query: queryObject });
}
...
<template>
  ...
  <h1 style="text-align: center; margin-top: 2rem;">Past Events</h1>
  ...
  <div class="eventSpace">
    ...
    <div v-else class="eventContainer">
      ...
      <h2 v-if="events.length === 0 && !error  && !isFetching && searchInput" style="margin-top: 3rem">{{noSearchResults}}</h2>
      <br/>

      <div v-for="(event, index) in filteredEvents" :key="event.id" style="margin-bottom: 1rem; width: 80%" id="eventFeed">
        <div v-if="event.id !== 'ad'">
          <EventComponent :event="event" :index="index" :currpath="historicPath" style="width:100%"/>
        </div>
